...
def dynamic_decode(decoder,
  ...
  """
  import tensorflow 
  if not isinstance(decoder, (tensorflow.contrib.seq2seq.Decoder, BaseDecoder)):
  ...
  with variable_scope.variable_scope(scope, "decoder") as varscope:
    ...
    # print('initial_inputs shape:', initial_inputs.shape.as_list())
    # print('initial finished shape:', initial_finished.shape.as_list())
    # print('initial_sequence_lengths shape:', initial_sequence_lengths.shape.as_list())
    
    def condition(unused_time, unused_outputs_ta, unused_state, unused_inputs,
                  finished, unused_sequence_lengths, unused_inputs_ta, unused_finished_ta, unused_sample_id_ta):
      return math_ops.logical_not(math_ops.reduce_all(finished))

    import tensorflow as tf 
    initial_input_ta = tensor_array_ops.TensorArray(
      dtype=tf.float32,
      size=0,
      dynamic_size=True)

    initial_dec_finished_ta = tensor_array_ops.TensorArray(
      dtype=tf.bool,
      size=0,
      dynamic_size=True)

    initial_sample_id_ta = tensor_array_ops.TensorArray(
      dtype=tf.int32,
      size=0, 
      dynamic_size=True)

    def body(time, outputs_ta, state, inputs, finished, sequence_lengths, input_ta, dec_finished_ta, sample_id_ta):
      ...
      return (time + 1, outputs_ta, next_state, next_inputs, next_finished,
              next_sequence_lengths, input_ta, dec_finished_ta, sample_id_ta)
    ...
    res = control_flow_ops.while_loop(
        ...
        loop_vars=(
            ...
            initial_input_ta,   # used for debug
            initial_dec_finished_ta,   # used for debug
            initial_sample_id_ta,
        ),
    ...
    final_input_ta = res[6].stack()   # used for debug
    final_dec_finished_ta = res[7].stack()    # used for debug
    final_sample_id_ta = res[8].stack()
  ...
  return final_outputs, final_state, final_sequence_lengths, final_input_ta, final_dec_finished_ta, final_sample_id_ta
