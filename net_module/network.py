...
class Network(object):
    def __init__(
        ...
        self.ptr_net1 = PointerNet(
            ...
            learning_rate=0.001, cell=tf.contrib.rnn.GRUCell, n_layers=n_layers, n_units=n_units, name=model_name + 'ptr_net_1')
        self.ptr_net2 = PointerNet(
            n_pointers=1, batch_size=batch_size, seq_max_len=seq_max_len, 
            learning_rate=0.001, cell=tf.contrib.rnn.GRUCell, n_layers=n_layers, n_units=n_units, name=model_name + 'ptr_net_2')
        ...
        with tf.variable_scope(model_name):
            ...
            with tf.variable_scope('hidden_state'):
                ...
                hid_state = tf.layers.dense(inputs=hid_state, units=512, activation=None)

            # 二维的，决定是飞还是打；第一维飞，第二维打
            # 决定每一个 batch 中 out_head1_action 所指定的两个动作是否都可用，至少飞是肯定可以选的
            with tf.variable_scope('head1_out'):
                out_head1_logits = tf.layers.dense(inputs=hid_state, units=2, use_bias=False, activation=tf.nn.relu)
            ...
            with tf.variable_scope('head2_out'):
                out_head2_logits = tf.layers.dense(inputs=hid_state, units=10, activation=tf.nn.relu)
            ...
            with tf.variable_scope('value'):
                value = tf.layers.dense(inputs=hid_state, units=256, activation=tf.nn.relu)
                self.value = tf.squeeze(tf.layers.dense(inputs=value, units=1, activation=tf.nn.relu), axis=-1)
